...
import LegalAssistant from "@/components/shared/ChatAssistant"; // Assuming the final name is LegalAssistant
...
export default function ResultPage() {
  ...
  useEffect(() => {
    // In a real Next.js app, useParams would work. Here, we simulate it.
    ...
    <div className="min-h-screen bg-slate-900 text-white">
      <header className="sticky top-0 z-40 bg-slate-900/70 backdrop-blur-lg border-b border-slate-800">
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between">
          <a href="/" className="
            relative inline-flex items-center gap-3 px-6 py-3 font-semibold text-white transition-all duration-300
            bg-slate-800/60 border border-slate-700 rounded-xl backdrop-blur-lg shadow-lg
            hover:bg-slate-800/80 hover:border-indigo-500/60 hover:shadow-indigo-500/20
            active:scale-95 focus:outline-none focus:ring-2 focus:ring-indigo-400/40
            glassmorphism
            ">Home</a>
          <div>
...
<motion.button
  ...
  whileHover={{ 
    ...
    borderColor: "rgba(99, 102, 241, 0.5)" // Corresponds to indigo-500
  }}
  whileTap={{ scale: 0.95 }}
  transition={{ type: "spring", stiffness: 300, damping: 20 }}
>
  {/* Subtle gradient glow that follows the mouse */}
  ...
  <span className="relative z-10">Find Missing Clauses</span>
      ...
      {/* AI Assistant is now a top-level component that manages its own visibility */}
